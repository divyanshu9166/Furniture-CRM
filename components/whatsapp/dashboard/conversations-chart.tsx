...
import type { ConversationsSeriesPoint } from '@/lib/wa-dashboard/types'
...
export function ConversationsChart({ series, loading, range, onRangeChange }: ConversationsChartProps) {
  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-surface shadow-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Conversations Over Time</h2>
          <p className="mt-0.5 text-xs text-muted">Daily message volume by direction</p>
        </div>
      ...
      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted">
...
function LineSvg({
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-md border border-border bg-surface px-2.5 py-1.5 text-[11px] shadow-lg"
