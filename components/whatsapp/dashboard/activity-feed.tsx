...
import type { ActivityItem, ActivityKind } from '@/lib/wa-dashboard/types'
        ...
        <>
          <ul className="divide-y divide-border">
            {visible.map((it, i) => {
              ...
              const stripe = i % 2 === 0 ? 'bg-transparent' : 'bg-surface-hover'
              const row = (
              ...
              return (
                <li key={it.id} className={cn(stripe, 'transition-colors hover:bg-surface-light')}>
                  {it.href ? (
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs">
            ...
            <div className="flex items-center gap-1">
              <span className="mr-1 text-muted">Show</span>
