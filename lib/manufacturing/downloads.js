...
async function handleMobileDownload(blob, filename) {
  ...
  // Fallback for desktop or unsupported mobile (e.g. WebViews blocking blob: URIs)
  // We upload the file temporarily to our server and then redirect to a GET endpoint
  // that serves it with Content-Disposition: attachment. This works perfectly with Android DownloadManager.
  try {
    const formData = new FormData()
    formData.append('file', new File([blob], filename, { type: blob.type }))
    formData.append('folder', 'temp-downloads')
    ...
    if (!res.ok) throw new Error('Failed to upload file for download')
    const { urls } = await res.json()
    
    if (urls && urls.length > 0) {
      const echoUrl = `/api/download-echo?url=${encodeURIComponent(urls[0])}&filename=${encodeURIComponent(filename)}`
      
      // We create a temporary <a> tag to trigger the GET download
    ...
    // Absolute fallback: open blob in new window (often blocked, but worth a try)
