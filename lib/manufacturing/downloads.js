...
export async function downloadBOMCSV(bom, isWorkSheet = false) {
  ...
  if (isWorkSheet) {
    rows.push(['Material', 'Size', 'SKU', 'Quantity', 'UoM'])
    bom.items?.forEach(i => {
      rows.push([
        i.rawMaterial?.name ?? '',
        i.rawMaterial?.description ?? '',
        i.rawMaterial?.sku ?? '',
        i.quantity,
        i.unitOfMeasure,
      ])
    ...
    rows.push(['Material', 'Size', 'SKU', 'Quantity', 'UoM', 'Wastage%', 'Unit Cost (₹)', 'Total Cost (₹)'])
    bom.items?.forEach(i => {
...
export async function downloadBOMPDF(bom, isWorkSheet = false) {
  ...
  try {
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 24px;">
        <thead>
          <tr style="background-color: #f1f5f9; text-align: left;">
            ...
            <th style="padding: 8px; border: 1px solid #cbd5e1;">Size</th>
    ...
    for (const item of (bom.items || [])) {
      ...
      html += `
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #cbd5e1; color: #475569;">${item.rawMaterial?.description || '—'}</td>
    ...
    if (!isWorkSheet) {
      html += `
        <tr style="background-color: #f8fafc; font-weight: bold;">
          <td colspan="8" style="padding: 8px; border: 1px solid #cbd5e1; text-align: right;">Total Material Cost</td>
    ...
    // Wait for EVERY <img> in the container to finish decoding before we snapshot
    // with html2canvas. A fixed timeout is unreliable when there are several
    // raw-material images (they decode from data URIs and can still be blank at
    // capture time), which is why raw-material images previously came out empty.
    const imgs = Array.from(container.querySelectorAll('img'))
    await Promise.all(imgs.map(img => {
      if (img.complete && img.naturalWidth > 0) return Promise.resolve()
      return new Promise(resolve => {
        img.onload = () => resolve()
        img.onerror = () => resolve() // never block the export on one bad image
      })
    }))
    // Small extra tick so the browser paints the decoded images.
